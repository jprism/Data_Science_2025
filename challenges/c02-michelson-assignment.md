...
error is less than the real error - Make a quantitative comparison
between Michelson’s uncertainty and his error. - The error in real life
is 2.972 times larger than the uncertainty that Michelson expected
...
**Observations**: Similarities - Some days are very grouped, some very
tight, means jusmp back and forth. Outliers outside of SD bars.
Differences - Tight clumps of data are more common in the real data.
Real data has the day with the most spread between the data points.
...
df_q2%>%
  ggplot() +
    geom_point(mapping=aes(Temp,VelocityVacuum))+
    geom_smooth(mapping=aes(Temp,VelocityVacuum))+
    labs(title="Temp VS VelocityVacuum")
```
...
df_q2 %>%
  ggplot() +
    geom_boxplot(mapping=aes(Distinctness,VelocityVacuum))+
    labs(title="Distinctness VS VelocityVacuum")
```
...
df_q2%>%
  ggplot() +
    geom_point(mapping=aes(Date,VelocityVacuum))+
    geom_smooth(mapping=aes(Date,VelocityVacuum))+
    labs(title="Date VS VelocityVacuum")
```
...
- Temp graph :
  - average increased with temp
  - most variance in the midrange
  - more data in the higher range
...
- Date:
  ...
  - the data taken in around june 20th had the tightest readings
