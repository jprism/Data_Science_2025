...
df_data <- 
  left_join(df_q4, df_pop, by = c("Geography" = "Geography"))
df_data
```
...
- Document your observations here.
  - 2 person families have confidence intervals of around 10-20k
  - 3 person families are similar with some counties have them an extra
    10-20k
  - 4, 5, and 6 person families have much wider ranges
  - The widest confidence interval is 5 person families for Nantucket,
    with the confidence interval covering over 200k in median household
    income
  - The median household income for 2 person families seems to be the
    smallest and generally the median value increases with family size
- Can you confidently distinguish between household incomes in Suffolk
  county? Why or why not?
  - No there is a lot of overlap of the different lines and some of the
    colors are very similar, like green and blue
...
``` r
df_data %>%
  ggplot(aes(population_estimate, income_SE, aes=.2)) +
  geom_point()+
  geom_smooth()+
...
- What *overall* trend do you see between `SE` and population? Why might
  this trend exist?
  - There seems to be a roughly sharp exponential relationship between
    SE and population where SE decreases as population increase. This
    makes sense as you require a high enough population to ensure you
    median isn’t being heavily influenced by the normal variation seen
    from the standard deviation of the data. This is supported by the
    trend line looking close to linear when log scales are used on the x
    and y axis.
- What does this *overall* trend tell you about the relative ease of
  studying small vs large counties?
  - It’s likely for more easy to make conclusive claims about larger
...
### how do states in the northeast compare to eachother in terms of family income
## im interested since I'm from the northeast
df_q8 <-
  df_data %>%
  mutate(state = sub(".*,\\s*", "", geographic_area_name))

df_q8 <-
  df_q8 %>%
  group_by(state, category) %>%
  summarise(
    average_income_estimate=mean(income_estimate, na.rm = TRUE),
    average_income_moe=mean(income_moe, na.rm = TRUE),
...
df_q8 %>%
  ...
  ggplot(aes(state, average_income_estimate, color = category)) +
    geom_point(position = position_dodge(width = wid))+
    coord_flip() +
    labs(
      x = "State",
...
- Main has the lowest household incomes
- Mass is the only state where 6 person families have the highest income
- in every state 2 person families have the lowest income
- rhode island, new jersey, connecticut and massachussets have the
  highest household incomes, specifically for 4,5, and 6 person
  families.
- New york where I’m from is pretty unremarkable in terms of incomes, it
  doesn’t have the lowest or the highest and has moderate spread between
